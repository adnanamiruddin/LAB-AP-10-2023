...
daya = int(input("Masukkan daya (VA): "))
pemakaian = int(input("Masukkan pemakaian (kWh): "))
...
match golongan:
    case "R1":
        if daya==900 : 
            ...
            print(f"Pemakaian = Jumlah Tagihan Anda sebesar: Rp,{1444.7*pemakaian:,.1f}".replace(",","x").replace(".",",").replace("x","."))
        elif daya==2200: 
            print(f"Pemakaian = Jumlah Tagihan Anda sebesar: Rp,{1444.7*pemakaian:,.1f}".replace(",","x").replace(".",",").replace("x","."))
        else:
